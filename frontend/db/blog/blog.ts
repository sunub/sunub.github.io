...
class Blog {
  ...
  async registerWatchers() {
    const rootPostsPath = (await findUpDir('posts')) ?? '';
    const watcher = chokidar.watch(rootPostsPath, {
      persistent: true,
      ignoreInitial: true,
    });
    watcher.on('change', changeFilePath => debounce(() => this.handleFileChange(changeFilePath), 100));
  }
  ...
  async handleFileChange(changeFilePath: string) {
    await this.#__withLock(async () => {
      ...
      if (!parsedCategory.success) {
        console.error(chalk.redBright(`카테고리가 잘못되었습니다. ${category}는 올바른 카테고리가 아닙니다.`));
        return;
      }
  ...
  async getPostContent(category: PostCategory, slug: string): Promise<string | null> {
    try {
      const rootPostsPath = (await findUpDir('posts')) ?? '';
      const filePath = path.join(rootPostsPath, category, `${slug}.mdx`);
  ...
  static async getInstance(rootPath: string = 'posts'): Promise<Blog> {
    ...
    Blog.initializationPromise = (async () => {
      try {
        ...
        if (!Blog.instance.isInitialized) {
          await Blog.instance.initialize(rootPath);
  ...
  async initialize(rootPath: string = 'posts'): Promise<void> {
    const categories: PostCategory[] = ['algorithm', 'code', 'cs', 'web'];

    const promisesOfTaskArrays = categories.map(category => this.processCategory(category, rootPath));
    const asyncIterableOfTaskArrays = toAsync(promisesOfTaskArrays);

    async function* flattenAsync<T>(iterable: AsyncIterable<T[] | undefined>): AsyncIterable<T> {
  ...
  private async processCategory(
    ...
    rootPath: string = 'posts'
  ): Promise<(() => Promise<void>)[] | undefined> {
    try {
      const rootPostsPath = (await findUpDir(rootPath)) ?? '';
      const categoryPath = path.join(rootPostsPath, category);
  ...
  private async processFile(category: PostCategory, filePath: string, slug: string): Promise<void> {
    try {
      const content = await readMdxFile(filePath);
      const parsed = parseFrontMatter(content);
...
const getBlogInstance = cache(async (rootPath: string = 'posts') => {
  if (global.__BLOG_INSTANCE__ && global.__BLOG_INSTANCE__.isInitialized) {
    return global.__BLOG_INSTANCE__;
  }

  global.__BLOG_INSTANCE__ = await Blog.getInstance(rootPath);
  return global.__BLOG_INSTANCE__;
});

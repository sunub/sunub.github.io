...
export class Post {
	...
	async getPostsByCategoryAndSlug(
		category: PostCategory,
		slug: string,
		findRootPath: string = "posts",
	) {
		await this.ensureInitialized(findRootPath);
		return this.processor.processFile(
			join(this.rootPath, category, `${slug}.mdx`),
		);
	}
	...
	async *postFrontMatterGenerator(
		batchSize: number = 10,
		filePath: "web" | "algorithm" | "code" | "cs" | "." = ".",
	) {
	...
	async _createFrontMatterIterator(
		filePath: "web" | "algorithm" | "code" | "cs" | "." = ".",
	) {
		...
		try {
			await this.ensureInitialized("posts");
			const filePaths = fx(
				this.getFileNames(join(this.rootPath, filePath)),
			).filter(

...
export async function getAlgorithmPostsFrontmatter(): Promise<
	PostFrontMatter[]
> {
	const allData = await getAllJsonFrontMatter();
...
export const getChunkedRecentPostsFrontmatter = cache(
	async (chunkSize = 10) => {
...
export const getPostFrontMatterByCategoryAndSlug = cache(
	async (category: PostCategory, slug: string) => {
		try {
			...
			const postData = currFrontMatter.find(
				(post) => post.frontmatter.slug === slug,
			);
			if (!postData) {
				throw new Error(
					`카테고리: ${category}, 슬러그: ${slug}에 해당하는 포스트를 찾을 수 없습니다.`,
				);
			}
			return postData;
		} catch (error) {

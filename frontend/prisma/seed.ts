...
async function seedPostFiles(data: CategoryPostData, chunkSize = 25) {
  const postData = data.map(post => {
    ...
    const { title, category, date, summary, slug } = frontmatter;
    return {
      title,
      category,
      date: new Date(date),
  ...
  const tagsData = data.flatMap(post => {
    const metadata = post;
    return metadata.frontmatter.tags.map((tag: string) => ({
      name: tag,
      postSlug: metadata.frontmatter.slug,
    }));
  });
...
async function createRedirectPaths() {
  ...
  const processCategory = (posts: CategoryPostData) => {
    for (const post of posts) {
      const { slug, category } = post.frontmatter;
...
async function cleanUpDB() {
  const cleanupSpinner = ora(`${chalk.bold(chalk.blueBright('loading'))}...\n`).start();
  try {
    console.time('🧹 Cleaned up the database...');
    await prisma.$transaction([prisma.tags.deleteMany(), prisma.post.deleteMany(), prisma.redirects.deleteMany()]);
...
async function seedingWebPost() {
  const webPostSpinner = ora(`${chalk.bold(chalk.blueBright('loading'))}...\n`).start();
  try {
...
async function seedingCsPost() {
  const csPostSpinner = ora(`${chalk.bold(chalk.blueBright('loading'))}...\n`).start();
  try {
...
async function seedingCodePost() {
  const codePostSpinner = ora(`${chalk.bold(chalk.blueBright('loading'))}...\n`).start();
  try {
...
async function seedingAlgoPost() {
  const algorithmPostSpinner = ora(`${chalk.bold(chalk.blueBright('loading'))}...\n`).start();
  try {
    console.time('📝 Created algorithm posts...');
    await seedPostFiles(allAlgorithmPosts);
    algorithmPostSpinner.succeed(chalk.green('Algorithm Post has been seeded!!'));
    console.timeEnd('📝 Created algorithm posts...');
  } catch (error) {
    algorithmPostSpinner.fail(chalk.redBright('Failed to seed algorithm posts'));
    console.error(error);
  }
}

async function seedingRedirectPath() {
  const redirectsSpinner = ora(`${chalk.bold(chalk.blueBright('loading'))}...\n`).start();
  try {

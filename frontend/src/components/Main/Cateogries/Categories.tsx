...
import { CategoryListWrapper, RootWrapper, Title, TitleWrapper } from './Categories.style';
    ...
    <RootWrapper>
      ...
      <CategoryListWrapper>
        {categories.map(category => (
          <Link className="blog-main__landing-page-link" href={`/post/${category}`} key={category}>
            <VisuallyHidden>{`${category} 카테고리로 이동합니다.`}</VisuallyHidden>
            {category}
          </Link>
        ))}
      </CategoryListWrapper>

...
const isPathCommandString = (value: unknown): value is string =>
	typeof value === "string" &&
	/^\s*[Mm]\s/.test(value) &&
	/[a-zA-Z]/.test(value);
...
export function WaveSvg({ ...delegated }) {
	...
	const safeEndSeq = createPathSequence(
		path.step1.end,
		path.step2.end,
		path.step3.end,
	);
	const safeMidSeq = createPathSequence(
		path.step1.mid,
		path.step2.mid,
		path.step3.mid,
	);
	const safeFrontSeq = createPathSequence(
		path.step1.front,
		path.step2.front,
		path.step3.front,
	);

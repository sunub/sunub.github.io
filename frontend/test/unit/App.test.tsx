...
import { render, screen, cleanup as unmountComponent } from '@testing-library/react';

vi.mock('db/blog/api');

const demoFrontMatters: FrontMatter[] = Array.from({ length: 10 }, (_, index) => ({
...
describe('블로그 메인 서버 컴포넌트 테스트', () => {
  ...
  test('최근 블로그 포스트 10개가 잘 렌더링 되는가?', async () => {
    vi.mocked(getRecentPostsMetadataInRange).mockResolvedValue(demoPublishedPosts);

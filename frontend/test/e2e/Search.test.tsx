...
test.describe('검색 접근성 테스트', () => {
  ...
  test('검색 열기 버튼 클릭 시 모달이 열리는가?', async ({ page }) => {
    ...
    await expect(rootDialogContainer.getByRole('dialog', { name: '검색 다이알로그 창' })).toBeAttached();
  });
  ...
  test('검색 모달 외부를 클릭할 시 모달이 닫히는가?', async ({ page }) => {
    ...
    await expect(rootDialogContainer.getByRole('dialog', { name: '검색 다이알로그 창' })).not.toBeAttached();
  });
  ...
  test('검색 모달을 ESC 버튼으로 닫을 수 있는가?', async ({ page }) => {
    ...
    const rootDialogContainer = page.getByTestId('blog-search__input-area');
    const dialog = rootDialogContainer.getByRole('dialog', { name: '검색 다이알로그 창' });
  ...
  test('검색 모달이 열릴 시 Input이 포커스 되는가?', async ({ page }) => {
    ...
    const dialog = rootDialogContainer.getByRole('dialog', { name: '검색 다이알로그 창' });
    await expect(dialog.getByRole('combobox', { name: '검색창' })).toBeVisible();
    await expect(dialog.getByRole('combobox', { name: '검색창' })).toBeFocused();
  });

  test('검색 모달이 열릴 시 포커스가 Modal 내부로 Lock 되는가?', async ({ page }) => {
    await expect(page.getByRole('button', { name: '검색' })).toBeEnabled();
  ...
  test('검색어를 타이핑 후 검색 결과가 있을 경우 상태가 적절히 변경이 되는가?', async ({ page }) => {
    await expect(page.getByRole('button', { name: '검색' })).toBeEnabled();
    ...
    await page.getByRole('combobox', { name: '검색창' }).fill(VALID_SEARCH_QUERY);
...
test.describe('검색 기능 테스트', () => {
  ...
  test('여러 결과를 포함하는 검색어 입력 시 적절한 결과가 나오는가?', async ({ page }) => {
    const modalOpenButton = page.getByRole('button', { name: '검색' });
    await expect(modalOpenButton).toBeEnabled();
    await expect(modalOpenButton).toBeVisible();
    await modalOpenButton.click();
    ...
    await expect(page.getByRole('option')).toHaveCount(VARIETY_SEARCH_QUERY_RESULT_COUNT, DEFAULT_TEST_OPTION);
  });
  ...
  test('검색어 결과가 없을 경우 적절한 UI가 나오는가?', async ({ page }) => {
    ...
    await expect(page.getByTestId('search-no-results')).toBeVisible(DEFAULT_TEST_OPTION);
  });
});
...
test.describe('검색 결과 키보드 네비게이션 테스트', () => {
  ...
  async function pressKeyDownAndVerifyNthOption(key: string, page: Page, nth: number) {
    const listbox = page.getByRole('listbox', { name: '검색 결과' });
    await expect(listbox).toBeVisible(DEFAULT_TEST_OPTION);
    ...
    await expect(nthOption).toHaveAttribute('aria-selected', 'true', DEFAULT_TEST_OPTION);

    await expect(listbox.getByRole('option', { selected: true })).toHaveCount(1, DEFAULT_TEST_OPTION);
  }
  ...
  test('Arrow Down/Up으로 검색 결과를 탐색할 수 있는가?', async ({ page }) => {
    ...
    await pressKeyDownAndVerifyNthOption('ArrowUp', page, VARIETY_SEARCH_QUERY_RESULT_COUNT - 1);
  });

  test('Arrow Down/Up으로 포커스를 이동한 후 Enter키를 통해 컨텐츠로 이동할 수 있는가?', async ({ page }) => {
    await pressKeyDownAndVerifyNthOption('ArrowDown', page, 0);
    await page.waitForTimeout(100);

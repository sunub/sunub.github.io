...
function reduceBase<A, Acc>(fn: (acc: Acc, a: A) => Acc, acc: Acc, iterable: Iterable<A>): Acc {
...
export function reduce<A, Acc>(fn: (acc: Acc, a: A) => Acc, iterator: Iterable<A>): IterableIterator<Acc>;
...
export function reduce<A, Acc>(
  ...
  if (iterable === undefined) {
    if (!isIterable(accOrIterable) && !isAsyncIterable(accOrIterable)) {
      throw new TypeError('인자로 Iterable 또는 AsyncIterable을 제공해야 합니다.');
    }
    ...
    return isIterable(iterable)
      ? reduceBase(fn as (acc: Acc, a: A) => Acc, accOrIterable as Acc, iterable)

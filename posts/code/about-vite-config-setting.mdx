...
`CORS`와 `프록시 서버`의 관계를 이해하기 위해, 먼저 웹 브라우저의 동작 방식부터 살펴보겠습니다.

브라우저의 `Same-Origin Policy`는 보안을 위해 다른 출처(도메인, 포트, 프로토콜)로의 HTTP 요청을 기본적으로 제한합니다. 동일 출처 정책(SOP) 는 브라우저는 자원의 출처를 비교할 때 세 가지를 기준으로 합니다
...
브라우저에서 실행되는 JavaScript 코드는 동일 출처 정책(SOP)에 따라 다른 출처의 자원에 직접 접근할 수 없습니다.
```

예를 들어, localhost:3000에서 실행되는 프론트엔드 애플리케이션이 api.example.com으로 직접 요청을 보내려고 할 경우 호스명이 일치하지 않기 때문에 브라우저는 이를 차단합니다.

위에서 설명한 SOP 정책의 3가지 조건이 모두 부합해야지만 `동일 출처`로 판단이 됩니다. 이러한 조건에 부합해 동일 출처로 판단이 되었을 경우 자원의 요청에 대해서 제한을 두지 않습니다. 그렇기 때문에 `브라우저(localhost:3000)` 에서 `프록시 서버(localhost:3000)`로 요청을 보내도 어떠한 문제도 일어나지 않고 자원을 요청하고 응답 받을 수 있습니다.

이때 `CORS (Cross-Origin Resource Sharing)`는 SOP 정책에 부합하지 않을 경우 SOP 제한을 우회하여 다른 출처의 리소스에 접근할 수 있도록 허용하는 메커니즘입니다. 서버는 응답 헤더를 통해 특정 출처에 대한 접근을 허용할 수 있으며, 이를 통해 브라우저는 다른 출처의 리소스를 안전하게 사용할 수 있게 됩니다. 주요 CORS 헤더는 다음과 같습니다.

```plaintext
Access-Control-Allow-Origin: 특정 출처 또는 *을 사용하여 모든 출처의 접근을 허용
Access-Control-Allow-Methods: 허용할 HTTP 메서드 (예: GET, POST)
Access-Control-Allow-Headers: 허용할 HTTP 헤더
Access-Control-Allow-Credentials: 자격 증명(쿠키, 인증 정보) 포함 여부
```

하지만 모든 외부 API 서버가 CORS를 올바르게 설정하고 있지는 않기 때문에, 로컬 개발 환경에서 외부 API에 접근할 때 CORS 오류가 발생할 수 있습니다.
...
여기서 프록시 서버가 중간에 포함될 경우 SOP 정책을 우회할 수 있습니다. 이에 대해서 더 자세하게 이야기 해보곘습니다.

우선 SOP는 브라우저에서 실행되는 JavaScript 코드가 다른 출처의 리소스에 접근하는 것을 제한하는 보안 정책입니다. 여기서 핵심은 `브라우저에서 실행되는 JavaScript 코드`라는 점입니다.

Vite 프록시 서버는 Node.js 환경에서 실행되는 서버 프로그램입니다. 서버 측 코드는 브라우저의 SOP 제약을 받지 않으며, 다른 출처의 리소스에 자유롭게 접근할 수 있습니다. 이러한 이유로 프록시 서버를 통해 브라우저에서 직접 요청을 보내는 것보다 더 많은 자유를 가질 수 있습니다.
...
( 필요에 따라 프록시 서버는 요청 및 응답 헤더를 조작하여 CORS 정책을 만족시키도록 합니다. )
↓
...
vite의 프록시 서버를 사용할 경우 위의 과정과 같은 과정을 거치게 됩니다. 이러한 과정을 통해서 브라우저의 SOP 정책을 우회하고 외부 서버에 대한 요청을 보낼 수 있게 되기 떄문에 우리는 자유롭게 외부 서버에 대한 요청을 보낼 수 있게 됩니다.

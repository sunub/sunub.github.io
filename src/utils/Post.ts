...
import { Typescript, Javascript, Web, Algorithm } from '@/components/icon/Category';
...
export async function getPost(): Promise<[Map<string, any>, Map<string, Description[]>]> {
    ...
    for (const [tag, files] of Object.entries(postFileData)) {
        for (const file of files) {
            const [description, content]: [Description, string] = divideDescriptionAndContent(file);
            ...
            !categories.has(description.category!) ? categories.set(description.category!, icon) : null;
        }
...
export function findPostByCategoryAndSlug(category: string, slug: string, blogpost: Map<string, Description[]>) {

...
export class FileProcessor {
	...
	async processFile(filePath: string): Promise<MatterTransformData | null> {
		...
		const settle = (resolve: (value: MatterTransformData | null) => void) => {
			if (settled) {
				return;
			}
			settled = true;
		...
		try {
			return await new Promise<MatterTransformData | null>(
				(resolve, reject) => {
					const onEnd = () => {
						settle(resolve);
					};
			...
			console.error(
				`파일 처리 중 오류 발생: ${filePath} - ${error instanceof Error ? error.message : String(error)}`,
			);
			return null;
		} finally {
